...
public class clsLanding
{
    ...
    public string SaveRecord(string name, string email,string countryCode, string phoneNo)
    {
        ...
        try
        {
            ...
            Qry = @"IF EXISTS (SELECT 1 FROM tblLandingPage WHERE email=@email)                        
                        ...
                        ELSE
                        BEGIN
                        INSERT INTO tblLandingPage(name,email,countryCode,phone,createdOn) VALUES(@name,@email,@countryCode,@phone,getdate())
                        SELECT 'S';
                        END";
